 ...
 */
function pm_parsePluginReadme($readmeTxtContents, $applyMarkdown = false){
	...
	if ( $applyMarkdown ){
		$sections = array_map('pm_applyMarkdown', $sections);
	}
 ...
 */
function pm_getPluginPackageMeta($packageInfo){
	if ( is_string($packageInfo) && file_exists($packageInfo) ){
		$packageInfo = pm_analysePluginPackage($packageInfo, true);
	}
 ...
 * @return string
 */
function pm_applyMarkdown($text){
 ...
 */
function pm_getPluginHeader( $file_data ) {
	...
	$plugin_data = pm_get_file_data( $file_data, $default_headers );
 ...
 */
function pm_get_file_data( $file_data, $all_headers ) {
	foreach ( $all_headers as $field => $regex ) {
		preg_match( '/' . preg_quote( $regex, '/' ) . ':(.*)$/mi', $file_data, ${$field});
		if ( !empty( ${$field} ) )
			${$field} = pm_cleanup_header_comment( ${$field}[1] );
		else
			${$field} = '';
	}
 ...
 * @return string
 */
function pm_cleanup_header_comment($str) {
